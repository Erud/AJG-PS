<#
.SYNOPSIS
   Enumerate all Trusts for a Domain
.DESCRIPTION
...
Write-output "Domain`tName`tDescription`tCreated`tModified`tDirection`tType`tAttributes`t"  
        ...
        If($? -and $ADDomainTrusts -ne $Null)
        {
           ...
           # Write-Output "Discovered $ADDomainTrustsCount trusts in $Domain"
            ...
            { 
                ...
                $TrustModified = $Trust.whenChanged 
                ...
                Write-output "$Domain`t$TrustName`t$TrustDescription`t$TrustCreated`t$TrustModified`t$TrustDirection`t$TrustType`t$TrustAttributes`t"
               # Write-output "`tTrust Description: $TrustDescription $TrustModified "
